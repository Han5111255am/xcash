...
**Xcash** 是最先进的开源、自托管的**加密货币收款网关**，面向商家、SaaS 产品、交易所和钱包平台，提供加密货币收款、USDT 收款和链上充值能力。

不同于 CoinGate、OpenNode 这类托管式支付处理商，Xcash 强调**完全自托管**：收款经智能合约直达你的多签钱包地址，Xcash 全程不过手资金，也不抽取平台手续费。它适合需要多链资产收款、充值和 Webhook 通知能力的业务系统。

**适用场景：** 电商加密货币收款、USDT 充值系统、跨境稳定币结算、SaaS 加密货币订阅计费、链上收款基础设施，以及企业内部数字资产入账管理。

## Xcash 有多安全？

打个比方，如果你用 Xcash 作为项目的加密货币收款网关，即使部署 Xcash 的服务器被攻破、数据库被拖库、密钥泄露，只要保证您的多签钱包地址不被篡改，您的资产都不会有任何安全风险，而且恢复服务后用户支付的款项，仍然乖乖的流入您的钱包，黑客无能为力。
...
资金路径（绿色）由智能合约写死，只在「买家 → 收款合约 → 你的多签钱包」之间流动；Xcash 仅作控制面，负责账单匹配、状态流转与通知，**不在资金路径上**。因此攻击者即便完全控制 Xcash 系统，也无法改写合约里写死的资金流向。

## 特性

| 特性 | 说明                              |
|------|---------------------------------|
| 账单支付 | 定额、定时的账单式收款，适合电商下单、订阅计费等场景      |
| 专属地址充币 | 为每个用户分配专属充币地址，随时转入、即时入账，体验同交易所  |
| 完全自托管 | 收款经智能合约直达你的钱包，Xcash 全程不过手资金     |
| 零平台手续费 | 不按交易抽成，只承担链上微量 Gas              |
| 多链多币种 | 覆盖主流 EVM 链与 Tron，支持任意 ERC-20 代币 |
| 多商户多项目 | 单实例隔离管理多个商户与项目                  |
| 合约账单 | EVM 链可为每张账单生成独立 CREATE2 收款地址    |
| 链上风控 | 接入 MistTrack 对支付、充币的来源地址做风险评分   |
| Webhook 回调 | 实时推送支付、充币事件                     |
...
- **支付**：账单式收款。每笔交易创建一张定额、定时的账单，买家付款后账单完成——适合电商下单、订阅计费等一次性收款场景。支付按地址分配方式分两种模式：
  - **差额支付**：买家共用收款地址，靠微小金额差额区分账单。全链通用、零额外部署成本；但相同币种、相同金额可并行的账单数受可用差额档位限制，高并发同价订单可能分配失败。
  - **合约支付**：EVM 专用，通过 CREATE2 为每张账单预测独立收款地址。地址互不冲突、天然支持高并发，金额也无需差额浮动。
- **充币**：交易所式充值。为每个用户分配专属充币地址，且多链共享，用户可随时转入、区块确认后入账，无需创建订单——适合需要维护用户余额的钱包、交易类业务。

## 链支持

| 功能 | ETH | BNB Chain | Arbitrum | Base | Tron | Polygon | Avalanche | Optimism | 其他 EVM |
|:----:|:---:|:---------:|:--------:|:----:|:----:|:-------:|:---------:|:--------:|:------:|
| 支付 | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 是 |  几乎都是  |
| 充值 | 是 | 是 | 是 | 是 | 否 | 是 | 是 | 是 |  几乎都是  |
...
| 性能模式 | 硬件配置 | 仅支付（未开启原生币扫描） | 开启原生币扫描（充值） |
|:-------:|:-------:|:----------------------:|:---------------------:|
...
系统默认关闭 EVM 原生币扫描。充值归集等链上交易依赖原生币扫描来感知和确认。请仅在 RPC 节点支持高频调用时，在管理后台 **系统 -> 平台参数** 中开启 **开启 EVM 原生币扫描**。
...
部署完成后，参考 [API 对接文档](API.md) 接入支付、充币和 Webhook 回调。
